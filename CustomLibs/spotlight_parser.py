...
class FileMetaDataListing:
    ...
    def ReadStrings(self, dont_decode=False):
        ...
        if dont_decode:
            ...
            strings = [x[:-2].decode('utf8', "backslashreplace") if x.endswith(b'\x16\x02') else x.decode('utf8',
                                                                                                          "backslashreplace")
                       for x in strings]
        self.pos += size
    ...
    def ParseItemV1(self, properties, id):
        ...
        while self.pos < self.size:
            ...
            if prop_index == 0:
                log.warning(
                    "Maybe something went wrong, skip index was 0 @ {} or rest of struct is slack".format(filepos))
                break
            ...
            if data_type & 0xF == 0x01:  # bool
                ...
                if data_len == 8:
                    ...
                    value = ", ".join([str(FileMetaDataListing.ConvertUint64ToSigned(self.ReadUint64())) for x in
                                       range(0, data_len // 8)])
            ...
            if prop == None:
                ...
                if value_type == 0:
                    ...
                    if prop_type & 2 == 2:  # == 0x0A:
                        ...
                        num_values = number >> 3
                        value = [FileMetaDataListing.ConvertUint64ToSigned(self.ReadVarSizeNum()[0]) for x in
                                 range(num_values)]
                        ...
                        if prop_type & 3 == 3:  # in (0x83, 0xC3, 0x03): # ItemKind
                                ...
                                for v in value:
                                    ...
                                    if cat == None:
                                        ...
                                        if len(all_translations) > 2:
                                            log.warning(
                                                'Encountered more than one control sequence in single translation'
                                                'string.')
                            ...
                            if value == None:
                                ...
                                for v in value:
                                    ...
                                    if cat == None:
                                        log.error(
                                            'error getting category for index={}  prop_type={}  prop_name={}'.format(v,
                                                                                                                     prop_type,
                                                                                                                     prop_name))
                                    else:
...
class SpotlightStore:
    ...
    def ParseCategoriesFromFileData(self, data_content, offsets_content, header_content):
        ...
        for index, offset in offsets:
            if offset >= len(data_content):
                log.error(
                    f'Index ({index}) Offset ({offset})> filesize ({len(data_content)}) in ParseCategoriesFromFileData()')
                continue
            entry_size, bytes_moved = SpotlightStore.ReadVarSizeNum(data_content[offset:])
    ...
    def ParseIndexesFromFileData(self, data_content, offsets_content, header_content, dictionary, has_extra_byte=False):
        ...
        for index, offset in offsets:
            ...
            if pos >= len(data_content):
                log.error(
                    f'Index ({index}) Offset ({offset})> filesize ({len(data_content)}) in ParseIndexesFromFileData()')
                continue
    ...
    def ProcessBlock(self, block, dictionary):
        if block.block_type == BlockType.UNKNOWN_0:
            ...
            pass
        elif block.block_type == BlockType.STRINGSV1:
            self.ParsePropertiesV1(block)
        ...
        for index in self.block0.indexes:
            ...
            try:
                ...
                if compressed_block.block_type & 0xFF != BlockType.METADATA:
                    log.error('Expected METADATA block, Unknown block type encountered: 0x{:X}'.format(
                    ...
                    if block_data[20:23] == b'bvx':
                        ...
                        if header in [b'bvx1', b'bvx2', b'bvxn']:
                            ...
                            if len(uncompressed) != uncompressed_size:
                                log.error(
                                    'Decompressed size does not match stored value, DecompSize={}, Should_be={}'.format(
            ...
            if self.version == 1:
                while (pos < meta_size):
                    ...
                    md_item = FileMetaDataListing(pos + 16, uncompressed[pos + 16: pos + 16 + item_size_2],
                                                  item_size_2 - 16)
                    try:
                        ...
                        if items_to_compare and self.ItemExistsInDictionary(items_to_compare, md_item):
                            ...
                            if existing_item != None:
                                log.warning(
                                    'Item already present id={}, name={}, existing_name={}'.format(md_item.id, name,
                                                                                                   existing_item[2]))
                                if existing_item[1] != md_item.parent_id:
                                    log.warning("Repeat item has different parent_id, existing={}, new={}".format(
                                        existing_item[1], md_item.parent_id))
                                ...
                                items[md_item.id] = [md_item.id, md_item.parent_id, md_item.GetFileName(), None,
    ...
    try:
        ...
        if store.is_ios_store:  # store.flags & 0x00010000 == 0x00010000:
            ...
            if os.path.isfile(data_path):
                ...
                log.error(
                    'Did not find file dbStr-1.map.data in the same folder as store.db. In order to parse this file' +
...
def main(input_path, output_folder, output_prefix=None):
    output_file_prefix = output_prefix if output_prefix else 'spotlight-store'

    # log
    log_level = logging.INFO  # Change this to logging.ERROR to suppress INFO logs as well
    ...
    log.setLevel(log_level)  # Set global log level to match
    ...
    if not os.path.exists(input_path):
        log.error("Input file '{}' does not exist".format(input_path))
        return

    ProcessStoreDb(input_path, output_folder, output_file_prefix)
